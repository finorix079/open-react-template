...
import { pendingPlans, generateSessionId, storePendingPlan } from '../chat/session';
    ...
    async start(controller) {
      ...
      try {
        ...
        if (actionablePlan.impossible) {
          writeResult(controller, {
            message: actionablePlan.message || 'Unable to process this query.',
            final: true,
        ...
        if (
          ...
          if (allReadOnly) {
            ...
            const result = await executeIterativePlanner(
              ...
              50,
              undefined,
              (info) => writeToolCall(controller, info),
            );
