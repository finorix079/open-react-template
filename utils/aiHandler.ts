...
import { clarifyAndRefineUserInput } from './queryRefinement';
	...
	pokemonService: {
		...
		async execute(input: any, parentObs: LangfuseObservation): Promise<unknown> {
			return executeWithObservation(parentObs, "pokemonService", input, async () => {
				...
				return await searchPokemon({ searchterm: (input as { searchterm: string }).searchterm, page: (input as { page: number }).page });
			});
		},
	...
	watchlistService: {
		...
		async execute(input: any, parentObs: LangfuseObservation): Promise<unknown> {
			return executeWithObservation(parentObs, "watchlistService", input, async () => {
				...
				return await watchlistList(input);
			});
		},
